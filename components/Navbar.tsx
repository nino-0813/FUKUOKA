    ...
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-700 ${scrolled ? 'bg-white/90 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-4 md:py-8'}`}>
      <div className="container mx-auto px-4 md:px-8 flex justify-start lg:justify-between items-center">
        <div className="flex flex-col">
          <span 
            className="text-[7px] md:text-[10px] tracking-[0.4em] font-black uppercase opacity-50 md:opacity-60" 
            style={{ 
              color: scrolled ? COLORS.text : 'white',
              textShadow: scrolled ? 'none' : '0 2px 4px rgba(0,0,0,0.5), 0 1px 2px rgba(0,0,0,0.7)'
            }}
          >
            Premium Atelier
          </span>
          <h1 
            className="text-[10px] md:text-lg font-black tracking-tighter leading-tight" 
            style={{ 
              color: scrolled ? COLORS.primary : 'white',
              textShadow: scrolled ? 'none' : '0 2px 6px rgba(0,0,0,0.6), 0 1px 3px rgba(0,0,0,0.8)'
            }}
          >
        ...
        <div className="hidden lg:flex items-center gap-12">
          {NAV_ITEMS.map(item => (
            <a 
              ...
              style={!scrolled ? { textShadow: '0 1px 3px rgba(0,0,0,0.5), 0 1px 2px rgba(0,0,0,0.7)' } : {}}
            >
              {item.label}
            </a>
          ))}
